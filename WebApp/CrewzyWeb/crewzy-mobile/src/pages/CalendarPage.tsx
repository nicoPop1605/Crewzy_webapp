...
const socket = io('https://192.168.1.108:4000');
...
export function CalendarPage() {
    ...
    useEffect(() => {
        ...
        fetch('[http://192.168.1.108:4000/graphql](http://192.168.1.108:4000/graphql)', {
            method: 'POST',
    ...
    const startFakerLoop = () => {
        fetch('http://192.168.1.108:4000/api/start-loop')
            .then(() => console.log("Faker Bot pornit cu succes!"))
            .catch((err) => console.error("Eroare: Serverul e oprit sau IP-ul e greșit.", err));
    };

    const stopFakerLoop = () => {
        fetch('http://192.168.1.108:4000/api/stop-loop')
            .then(() => console.log("Faker Bot oprit!"))
            .catch((err) => console.error("Eroare: Serverul e oprit sau IP-ul e greșit.", err));
    };
        ...
        <div className="flex-1 flex flex-col overflow-auto">
            ...
            <div className="bg-white/60 backdrop-blur-md border-b border-gray-200 px-8 py-4">
                ...
                <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 mt-4">
                    {/* Rândul 1 (pe mobil): Butonul My Plans și Bara de căutare */}
                    <div className="flex items-center w-full md:w-auto gap-2 md:gap-4 flex-1">
                        <button className="whitespace-nowrap px-4 py-2 bg-gray-100 text-gray-900 rounded-lg text-sm md:text-base font-medium hover:bg-gray-200 transition-colors">
                            My Plans
                        </button>
                        <div className="flex-1 relative max-w-md">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 md:w-5 md:h-5 text-gray-400" />
                            <input type="text" placeholder="Search events" className="w-full pl-9 md:pl-10 pr-3 md:pr-4 py-2 text-sm md:text-base bg-white border border-gray-200 rounded-lg outline-none focus:border-gray-400 transition-colors" />
                        </div>
                    </div>

                    {/* Rândul 2 (pe mobil): Butoanele pentru Bot */}
                    <div className="flex w-full md:w-auto gap-2 md:ml-auto">
                        <button onClick={startFakerLoop} className="flex-1 md:flex-none px-3 md:px-4 py-2 bg-indigo-100 text-indigo-700 font-semibold rounded-lg hover:bg-indigo-200 transition text-xs md:text-sm whitespace-nowrap">
                            Start Faker 🤖
                        </button>
                        <button onClick={stopFakerLoop} className="flex-1 md:flex-none px-3 md:px-4 py-2 bg-red-100 text-red-700 font-semibold rounded-lg hover:bg-red-200 transition text-xs md:text-sm whitespace-nowrap">
                            Stop Bot 🛑
                        </button>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-auto p-4 md:p-8">
                ...
                {/* Quick Action Banner */}
                <div className="bg-gradient-to-r from-purple-500 to-blue-500 rounded-xl md:rounded-2xl p-4 md:p-6 mb-4 md:mb-6 shadow-lg">
                    {/* Am folosit flex-col pe mobil și md:flex-row pe web */}
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0">

                        <div className="flex items-center gap-3 md:gap-4">
                            {/* Am micșorat cercul și iconița Zap pe mobil */}
                            <div className="w-10 h-10 md:w-16 md:h-16 shrink-0 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center">
                                <Zap className="w-5 h-5 md:w-8 md:h-8 text-white" />
                            </div>
                            <div className="text-white">
                                {/* Am micșorat textul pe mobil */}
                                <h3 className="text-base md:text-xl font-bold mb-0.5 md:mb-1">Free Today?</h3>
                                <p className="text-white/90 text-xs md:text-sm">Let your friends know you're available to hang out!</p>
                            </div>
                        </div>

                        {/* Butonul: pe mobil e mai mic, dar pe toată lățimea (w-full). Pe web revine la dimensiunea normală */}
                        <button onClick={markTodayFree} className="w-full md:w-auto px-4 py-2 md:px-8 md:py-3 bg-white text-purple-600 rounded-lg md:rounded-xl font-semibold text-sm md:text-base hover:bg-gray-50 transition-all shadow-md md:shadow-lg">
                            I'm Free Today!
                        </button>
                ...
                <div className="bg-white/60 backdrop-blur-md rounded-3xl shadow-xl mb-6">
                    <div className="p-8">
                        <div className="flex items-center justify-between mb-4 md:mb-8">
                            <div>
                                <h2 className="text-2xl md:text-4xl font-bold text-gray-900">{monthName} {year}</h2>
                                {/* Clasa "hidden md:block" face ca textul de mai jos sa dispară DOAR pe telefon */}
                                <p className="hidden md:block text-gray-600 text-lg mt-1">Mark your free days and see who's available</p>
                            </div>

                            <div className="flex items-center gap-1 md:gap-2">
                                <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1))} className="p-1.5 md:p-3 hover:bg-gray-100 rounded-lg transition-colors">
                                    <ChevronLeft className="w-5 h-5 md:w-6 md:h-6" />
                                </button>
                                <button onClick={() => setCurrentDate(new Date())} className="px-3 py-1.5 md:px-6 md:py-3 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium transition-colors text-sm md:text-lg">
                                    Today
                                </button>
                                <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1))} className="p-1.5 md:p-3 hover:bg-gray-100 rounded-lg transition-colors">
                        ...
                        <div className="grid grid-cols-7 gap-2 md:gap-4">
                            {/* Lista cu zile în format scurt și lung pentru responsive */}
                            {[
                                { scurt: 'Sun', lung: 'Sunday' },
                                { scurt: 'Mon', lung: 'Monday' },
                                { scurt: 'Tue', lung: 'Tuesday' },
                                { scurt: 'Wed', lung: 'Wednesday' },
                                { scurt: 'Thu', lung: 'Thursday' },
                                { scurt: 'Fri', lung: 'Friday' },
                                { scurt: 'Sat', lung: 'Saturday' }
                            ].map((day) => (
                                <div key={day.lung} className="text-center font-bold text-gray-700 py-3 text-xs md:text-base">
                                    {/* Pe mobil (default) arătăm varianta scurtă, dar o ascundem pe ecrane medii/mari (md:hidden) */}
                                    <span className="md:hidden">{day.scurt}</span>
                                    {/* Pe mobil o ascundem (hidden), dar o arătăm pe ecrane medii/mari (md:inline) */}
                                    <span className="hidden md:inline">{day.lung}</span>
                                </div>
                            ))}
                            ...
                            {days.map((day, index) => {
                                ...
                                return (
                                    <button key={day} onClick={() => handleDayClick(day)} className={`aspect-square rounded-2xl p-2 md:p-4 relative transition-all flex flex-col items-start ${today ? 'ring-2 md:ring-4 ring-gray-900' : ''} ${status?.status === 'free' ? 'bg-green-100 hover:bg-green-200' : ''} ${status?.status === 'maybe' ? 'bg-yellow-100 hover:bg-yellow-200' : ''} ${!status ? 'hover:bg-gray-100' : ''}`}>
                                        <div className="text-base md:text-xl font-semibold text-gray-900 mb-1">{day}</div>
                                        <div className="flex flex-col gap-1 w-full mt-auto">
                                            {eventsToday.slice(0, 2).map(ev => (
                                                <div key={ev.id} className={`w-full text-left truncate text-[10px] md:text-xs font-medium px-1 md:px-2 py-0.5 md:py-1 rounded text-white ${ev.color}`}>
                                                    {ev.title}
                                                </div>
                                            ))}
                                        </div>
                                        {status && (
                                            <div className="absolute top-1 right-1 md:top-2 md:right-2">
                                                <UserCheck className={`w-3 h-3 md:w-4 md:h-4 ${status.status === 'free' ? 'text-green-600' : 'text-yellow-600'}`} />
                                            </div>
                                        )}
                                    </button>
                                );
                ...
                <div className="bg-white/60 backdrop-blur-md rounded-2xl shadow-lg p-3 md:p-6 mb-6">
                    <div className="flex flex-wrap items-center justify-center gap-3 md:gap-8 text-xs md:text-base">

                        <div className="flex items-center gap-1 md:gap-2">
                            <div className="w-3 h-3 md:w-5 md:h-5 bg-green-100 rounded border border-green-200" />
                            <span className="text-gray-700 font-medium whitespace-nowrap">I'm Free</span>
                        </div>

                        <div className="flex items-center gap-1 md:gap-2">
                            <div className="w-3 h-3 md:w-5 md:h-5 bg-yellow-100 rounded border border-yellow-200" />
                            <span className="text-gray-700 font-medium whitespace-nowrap">Maybe Free</span>
                        </div>

                        <div className="flex items-center gap-1 md:gap-2">
                            <div className="w-3 h-3 md:w-5 md:h-5 rounded border-2 md:border-4 border-gray-900" />
                            <span className="text-gray-700 font-medium whitespace-nowrap">Today</span>
                        </div>

                        <div className="flex items-center gap-1 md:gap-2">
                            <Users className="w-3 h-3 md:w-5 md:h-5 text-gray-600" />
                            <span className="text-gray-700 font-medium whitespace-nowrap">Friends Available</span>
                        </div>
